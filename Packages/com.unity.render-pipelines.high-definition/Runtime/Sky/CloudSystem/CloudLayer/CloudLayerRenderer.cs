...
namespace UnityEngine.Rendering.HighDefinition
{
    class CloudLayerRenderer : CloudRenderer
    {
        ...
        ComputeShader m_BakeCloudTextureCS, m_BakeCloudShadowsCS;
        int m_BakeCloudTextureKernel, m_BakeCloudShadowsKernel;
        ...
        public override void Build()
        {
            ...
            m_BakeCloudTextureCS = shaders.bakeCloudTextureCS;
            m_BakeCloudTextureKernel = m_BakeCloudTextureCS.FindKernel("BakeCloudTexture");

            m_BakeCloudShadowsCS = shaders.bakeCloudShadowsCS;
            m_BakeCloudShadowsKernel = m_BakeCloudShadowsCS.FindKernel("BakeCloudShadows");
        }
        ...
        public override void Cleanup()
        {
            ...
            m_BakeCloudTextureCS = null;
            m_BakeCloudShadowsCS = null;

            if (m_PrecomputedData != null)
            {
                PrecomputationCacheInstance.Release(m_LastPrecomputationParamHash);
        ...
        bool UpdateCache(CloudLayer cloudLayer, Light sunLight)
        {
            ...
            {
                PrecomputationCacheInstance.Release(m_LastPrecomputationParamHash);
                m_PrecomputedData = PrecomputationCacheInstance.Get(cloudLayer, currPrecomputationParamHash);
        ...
        public override void RenderSunLightCookie(BuiltinSunCookieParameters builtinParams)
            => m_PrecomputedData.BakeCloudShadows(this, (CloudLayer)builtinParams.cloudSettings, builtinParams.sunLight, builtinParams.hdCamera, builtinParams.commandBuffer);
        ...
        public override void RenderClouds(BuiltinSkyParameters builtinParams, bool renderForCubemap)
        {
            ...
            m_PrecomputedData.InitIfNeeded(this, cloudLayer, builtinParams.sunLight, hdCamera, builtinParams.commandBuffer);
        ...
        class PrecomputationCache
        {
            ...
            public void Clear()
            {
                foreach (var refCountData in m_CachedData.Values)
                {
                    refCountData.refCount = 0;
                    refCountData.data.Release();
                    m_DataPool.Release(refCountData);
                }
        ...
        class PrecomputationData
        {
            ...
            static TextureCache s_CloudTextureCache;
            static TextureCache s_CloudShadowsCache;

#if UNITY_EDITOR
            [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.SubsystemRegistration)]
            static void ResetStaticsOnLoad()
            {
                s_CloudTextureCache.Clear();
                s_CloudShadowsCache.Clear();
            }
#endif
            ...
            public void Allocate(CloudLayer cloudLayer)
            {
                ...
                if (!s_CloudTextureCache.TryGet(cloudTextureWidth, cloudTextureHeight, ref cloudTextureRT))
                ...
                if (cloudLayer.CastShadows && !s_CloudShadowsCache.TryGet(cloudShadowsResolution, cloudShadowsResolution, ref cloudShadowsRT))
            ...
            public void Release()
            {
                s_CloudTextureCache.Cache(cloudTextureHeight, cloudTextureHeight, cloudTextureRT);
                s_CloudShadowsCache.Cache(cloudShadowsResolution, cloudShadowsResolution, cloudShadowsRT);
            }

            public bool InitIfNeeded(CloudLayerRenderer cloudLayerRenderer, CloudLayer cloudLayer, Light sunLight, HDCamera hdCamera, CommandBuffer cmd)
            {
                if (initialized) return false;

                var bakeCloudTextureCS = cloudLayerRenderer.m_BakeCloudTextureCS;
                var bakeCloudTextureKernel = cloudLayerRenderer.m_BakeCloudTextureKernel;
                ...
                cmd.SetComputeVectorParam(bakeCloudTextureCS, HDShaderIDs._Params, params1);
                cmd.SetComputeTextureParam(bakeCloudTextureCS, bakeCloudTextureKernel, _CloudTexture, cloudTextureRT);

                cmd.SetComputeTextureParam(bakeCloudTextureCS, bakeCloudTextureKernel, _CloudMapA, cloudLayer.layerA.cloudMap.value);
                var paramsA = cloudLayer.layerA.GetBakingParameters();

                if (cloudLayer.NumLayers == 1)
                {
                    bakeCloudTextureCS.DisableKeyword("USE_SECOND_CLOUD_LAYER");
                    cmd.SetComputeVectorParam(bakeCloudTextureCS, HDShaderIDs._Params1, paramsA.Item1);
                    cmd.SetComputeVectorParam(bakeCloudTextureCS, HDShaderIDs._Params2, paramsA.Item2);
                }
                else
                {
                    cmd.SetComputeTextureParam(bakeCloudTextureCS, bakeCloudTextureKernel, _CloudMapB, cloudLayer.layerB.cloudMap.value);
                    var paramsB = cloudLayer.layerB.GetBakingParameters();

                    bakeCloudTextureCS.EnableKeyword("USE_SECOND_CLOUD_LAYER");
                    s_VectorArray[0] = paramsA.Item1; s_VectorArray[1] = paramsB.Item1;
                    cmd.SetComputeVectorArrayParam(bakeCloudTextureCS, HDShaderIDs._Params1, s_VectorArray);
                    s_VectorArray[0] = paramsA.Item2; s_VectorArray[1] = paramsB.Item2;
                    cmd.SetComputeVectorArrayParam(bakeCloudTextureCS, HDShaderIDs._Params2, s_VectorArray);
                }
                ...
                cmd.DispatchCompute(bakeCloudTextureCS, bakeCloudTextureKernel, threadGroupX, threadGroupY, 1);

                initialized = true;
                return true;
            }

            public void BakeCloudShadows(CloudLayerRenderer cloudLayerRenderer, CloudLayer cloudLayer, Light sunLight, HDCamera hdCamera, CommandBuffer cmd)
            {
                InitIfNeeded(cloudLayerRenderer, cloudLayer, sunLight, hdCamera, cmd);

                var bakeCloudShadowsCS = cloudLayerRenderer.m_BakeCloudShadowsCS;
                var bakeCloudShadowsKernel = cloudLayerRenderer.m_BakeCloudShadowsKernel;

                Vector4 _Params = cloudLayer.shadowTint.value;
                _Params.w = cloudLayer.shadowMultiplier.value * 8.0f;

                // Parameters
                cmd.SetComputeFloatParam(bakeCloudShadowsCS, HDShaderIDs._Resolution, 1.0f / cloudShadowsResolution);
                cmd.SetComputeVectorParam(bakeCloudShadowsCS, HDShaderIDs._Params, _Params);

                cmd.SetComputeTextureParam(bakeCloudShadowsCS, bakeCloudShadowsKernel, _CloudTexture, cloudTextureRT);
                cmd.SetComputeTextureParam(bakeCloudShadowsCS, bakeCloudShadowsKernel, _CloudShadows, cloudShadowsRT);
                ...
                s_VectorArray[0] = _FlowmapParamA; s_VectorArray[1] = _FlowmapParamB;
                cmd.SetComputeVectorArrayParam(bakeCloudShadowsCS, HDShaderIDs._FlowmapParam, s_VectorArray);

                s_VectorArray[0] = sunLight.transform.right; s_VectorArray[1] = sunLight.transform.up;
                s_VectorArray[0].w = cloudLayer.layerA.altitude.value; s_VectorArray[1].w = cloudLayer.layerB.altitude.value;
                cmd.SetComputeVectorArrayParam(bakeCloudShadowsCS, HDShaderIDs._Params1, s_VectorArray);

                cmd.SetComputeVectorParam(bakeCloudShadowsCS, HDShaderIDs._SunDirection, -sunLight.transform.forward);

                cmd.SetComputeTextureParam(bakeCloudShadowsCS, bakeCloudShadowsKernel, _FlowmapA, cloudLayer.layerA.flowmap.value);
                cmd.SetComputeTextureParam(bakeCloudShadowsCS, bakeCloudShadowsKernel, _FlowmapB, cloudLayer.layerB.flowmap.value);

                // Keywords
                bool enabled1 = cloudLayer.layerA.castShadows.value;
                var mode1 = cloudLayer.layerA.distortionMode.value;
                CoreUtils.SetKeyword(bakeCloudShadowsCS, "LAYER1_OFF", !enabled1);
                CoreUtils.SetKeyword(bakeCloudShadowsCS, "LAYER1_STATIC", enabled1 && mode1 == CloudDistortionMode.None);
                CoreUtils.SetKeyword(bakeCloudShadowsCS, "LAYER1_PROCEDURAL", enabled1 && mode1 == CloudDistortionMode.Procedural);
                CoreUtils.SetKeyword(bakeCloudShadowsCS, "LAYER1_FLOWMAP", enabled1 && mode1 == CloudDistortionMode.Flowmap);

                bool enabled2 = (cloudLayer.layers.value == CloudMapMode.Double) && cloudLayer.layerB.castShadows.value;
                var mode2 = cloudLayer.layerB.distortionMode.value;
                CoreUtils.SetKeyword(bakeCloudShadowsCS, "LAYER2_OFF", !enabled2);
                CoreUtils.SetKeyword(bakeCloudShadowsCS, "LAYER2_STATIC", enabled2 && mode2 == CloudDistortionMode.None);
                CoreUtils.SetKeyword(bakeCloudShadowsCS, "LAYER2_PROCEDURAL", enabled2 && mode2 == CloudDistortionMode.Procedural);
                CoreUtils.SetKeyword(bakeCloudShadowsCS, "LAYER2_FLOWMAP", enabled2 && mode2 == CloudDistortionMode.Flowmap);
                ...
                cmd.DispatchCompute(bakeCloudShadowsCS, bakeCloudShadowsKernel, threadGroupX, threadGroupY, 1);
                cloudShadowsRT.rt.IncrementUpdateCount();
            }
        }

        static PrecomputationCache s_PrecomputationCache = null;
        static PrecomputationCache PrecomputationCacheInstance => s_PrecomputationCache ??= new PrecomputationCache();
        ...
        static void ResetStaticsOnLoad()
        {
            s_PrecomputationCache?.Clear();
            s_PrecomputationCache = null;
        }
#endif
    }
}
